...
from torchvision.models import resnet18, resnet50, resnet34, ResNet34_Weights, ResNet50_Weights, ResNet18_Weights
...
class WeatherModelRes50DeepFc(nn.Module):
    ...
    def forward(self,x):
        ...
        time=self.time(out)
        return weather,time
class WeatherModelRes34DeepFc(nn.Module):
    def __init__(self):
        super(WeatherModelRes34DeepFc,self).__init__()
        # res=resnet18()
        # res=resnet50()
        # res=resnet34(pretrained=True)
        res=resnet34(weights=ResNet34_Weights.DEFAULT)
        res.fc=nn.Identity()    #恒等函数，即该层fc不做变化，（等会可以输出网络结构看看）
        self.res=res
        self.weather=nn.Sequential(
            nn.Linear(512,128),
